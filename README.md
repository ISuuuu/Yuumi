...
- **战绩查询** — 搜索任意召唤师，查看对局历史与详情（支持 10 人数据、经典模式、海克斯大乱斗战绩及海克斯强化解析）
...
│   │   ├── tools/        # 工具箱模块（自定义房间/符文配置/自动流程卡片/战利品批量处理）
│   │   ├── layout/       # 布局与自定义标题栏
...
│   │   ├── lcu/          # LCU 连接（进程检测、HTTPS 代理、WebSocket、OP.GG Client、SGP并发限流、资源预加载）
│   │   ├── parsers/      # 数据清洗（召唤师/战绩/对局信息/TFT云顶之弈）
│   │   ├── agents/       # 自动化任务（BP/自动接受/自动重连/自动荣誉/接受邀请）
...
| 分类            | 内容                                                     |
| --------------- | -------------------------------------------------------- |
| General         | 客户端路径、启动选项、HTTP 代理、日志、上传 API、SignalR  |
| Personalization | 主题色、语言、胜/败/重赛卡片颜色、侧边栏菜单显示/隐藏    |
| Functions       | 自动化流程开关（BP/匹配/重连/荣誉/邀请/上传等）+ 候选列表 |
| Other           | 公告记录、搜索历史                                       |
