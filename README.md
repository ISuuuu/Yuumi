...
- **战绩查询与对局回放** — 搜索任意召唤师，查看对局历史与详情（支持 10 人数据、经典模式、海克斯大乱斗战绩及海克斯强化解析），支持一键下载对局 ROFL 回放与调用客户端启动观看
- **生涯与英雄熟练度** — 当前召唤师近期战绩概览、资产统计、英雄熟练度 2.0 追踪（里程碑评级、成就积分、等级排行）与挑战勋章自由装配配置
- **对局信息与宿命对局** — 实时显示当前对局 10 人段位、近期 KDA、阵营组队分析；支持历史多局深度交手回溯检测、多次相遇统计及徽标优先级互斥展示（曾友/曾敌）
- **路人集 (曾同局玩家)** — 自动记录每局同场相遇玩家与频次，支持自定义标签/备注标记（如黑名单、绝活哥等）、标记置顶与筛选、选人/对局聊天室自动提醒己方标记玩家，支持 JSON 导入导出与自动身份回填
- **实用工具箱** — 包含基于战绩的好友清理（智能筛选长期未开黑/未上线好友并批量删除）、OP.GG 符文与出装方案一键导入装备页、直接观战指定玩家、自定义 5v5 房间创建、在线状态与生涯背景设置、客户端缩放与黑边修复、游戏设置防篡改锁定等
...
│   │   ├── career/       # 生涯战绩模块（召唤师信息/战绩页签/英雄熟练度与挑战/战利品入口）
│   │   ├── search/       # 战绩查询模块（战绩列表项/迷你列表/对局详情面板）
│   │   ├── settings/     # 设置页功能区块（个性化/云端服务/更新/关于等，含共享样式与自动保存 hook）
│   │   ├── opgg/         # OP.GG 弹窗子组件（筛选栏/梯度榜/出装攻略面板 + 共享格式化工具）
│   │   ├── tools/        # 工具箱模块（好友清理/自定义房间/符文出装/自动流程/观战，quickactions/ 为快捷操作卡片）
│   │   ├── layout/       # 布局与自定义标题栏
│   │   ├── ReplayButton.vue # 对局回放下载与启动观看按钮
...
│   ├── composables/      # 组合式 hooks（useLcuAsset/useToast/usePlayerSearch/useTftData/useLoot/useFateBadge 等）
...
├── src-tauri/            # Rust 后端
│   ├── src/
│   │   ├── main.rs       # 程序入口：日志初始化、便携升级判定、rustls CryptoProvider 配置
│   │   ├── lib.rs        # 入口：AppState、命令注册、系统托盘
│   │   ├── logging.rs    # 日志初始化
│   │   ├── config.rs     # 配置读写与 Schema 无缝迁移
│   │   ├── saved_players.rs # 路人集 (曾同局玩家 SQLite 持久化与标记)
│   │   ├── runtime.rs    # 运行时模式检测 (便携版/安装版)
│   │   ├── portable_updater.rs # 便携版专用更新与热替换助手
...
│   │   ├── tools.rs      # 杂项工具（回放管理/好友清理/装备页导入/符文/OP.GG数据抓取/观战）
