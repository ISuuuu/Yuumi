...
fn compare_names(name_a: &str, name_b: &str) -> bool {
    let clean_a = name_a.split('#').next().unwrap_or(name_a).trim().to_lowercase();
    let clean_b = name_b.split('#').next().unwrap_or(name_b).trim().to_lowercase();
    clean_a == clean_b
}
        ...
        loop {
            ...
            if active_player_name.is_none() {
                match client.get("https://127.0.0.1:2999/liveclientdata/activeplayername").send().await {
                    ...
                    Err(e) => {
                        log::debug!("尝试连接游戏内 Live Client Data 端口失败 (游戏可能未完全载入): {}", e);
                    }
                }
            }
                        ...
                        match resp.json::<LiveEventsResponse>().await {
                            Ok(events_resp) => {
                                ...
                                if last_processed_event_id == -1 {
                                    last_processed_event_id = events_resp.Events.iter().map(|e| e.EventID).max().unwrap_or(-1);
                                    log::info!("游戏内事件监控初始化，当前最大 EventID: {}", last_processed_event_id);
                                }
                                ...
                                for event in events_resp.Events {
                                    if event.EventID > last_processed_event_id {
                                        ...
                                        if event.EventName == "Multikill" {
                                            if let (Some(killer), Some(streak)) = (&event.KillerName, event.KillStreak) {
                                                if compare_names(killer, player_name) && cfg.screenshot_on_multikill_levels.contains(&streak) {
                                                    should_capture = true;
                                        ...
                                        if event.EventName == "Legendary" {
                                            if let Some(killer) = &event.LegendaryTo {
                                                if compare_names(killer, player_name) && cfg.screenshot_on_multikill_levels.contains(&8) {
                                                    should_capture = true;
                                        ...
                                        if should_capture {
                                            log::info!("检测到符合截图条件的事件: {}，触发异步截图...", capture_reason);
                                            ...
                                            tokio::spawn(async move {
                                                if let Err(e) = execute_screenshot(app_clone, &player_name_clone, &capture_reason, &custom_save_path).await {
...
async fn execute_screenshot(
    ...
    let filename = format!("{}_{}_{}.png", reason, player_name, now.format("%Y%m%d_%H%M%S"));
    save_dir.push(filename);
    let save_path_str = save_dir.to_string_lossy().to_string();

    // 3. 截取屏幕数据：优先定位 LOL 游戏窗口所在的显示器并截取该显示器，如无窗口则截取主屏幕
    let img_buffer = tokio::task::spawn_blocking(move || -> Result<xcap::image::RgbaImage, String> {
        use xcap::{Window, Monitor};
        let windows = Window::all().map_err(|e| e.to_string())?;
        // 匹配 LOL 游戏窗口
        let lol_window = windows.into_iter().find(|w| w.title() == "League of Legends (TM) Client");
        ...
        let main_monitor = monitors.first().ok_or_else(|| "未检测到任何可用显示器".to_string())?;
        let img = main_monitor.capture_image().map_err(|e| e.to_string())?;
        Ok(img)
    }).await.map_err(|e| format!("截图子线程异常: {}", e))??;

    // 4. 将图像二进制数据写入本地磁盘保存为 PNG
    tokio::task::spawn_blocking(move || -> Result<(), String> {
        img_buffer.save(&save_dir).map_err(|e| format!("写入磁盘失败: {}", e))
    }).await.map_err(|e| format!("保存子线程异常: {}", e))??;
