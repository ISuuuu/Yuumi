...
async function loadPerkData() {
  try {
    const stylesResp = await lcuRequest<any>(
      "GET",
  ...
  try {
    const perksResp = await lcuRequest<any[]>(
      "GET",
...
const primaryStyle = computed(() => {
  ...
  return (
    perkStyles.value.find(
      (s: any) => s.id === activeRune.value.primary_page_id,
    ) || null
  );
});
...
const secondaryStyle = computed(() => {
  ...
  return (
    perkStyles.value.find(
      (s: any) => s.id === activeRune.value.secondary_page_id,
    ) || null
  );
});
...
<template>
  <div class="opgg-panel">
    ...
    >
      ...
      >
        <LcuImage
          :src="getChampIcon(buildData.summary.id || selectedChampId!)"
          class="build-champ-icon"
        />
        <div class="build-title-info">
          <div class="build-title-name">
            {{
              championsMap.get(buildData.summary.id || selectedChampId!) ||
              buildData.summary.name ||
              "英雄"
            }}
          </div>
          <div class="build-title-pos" v-if="selectedPosition">
            {{ selectedPosition }}
          </div>
        </div>
      ...
      <!-- 召唤师技能：左右并排显示两组推荐 -->
      <div
        v-if="buildData.summoner_spells?.length"
        class="build-card spell-card"
      >
        <div class="spell-presets-container">
          <div
            class="spell-pair"
            v-for="(s, i) in buildData.summoner_spells.slice(0, 2)"
            :key="i"
          >
        ...
        <div class="rune-tree-container">
              ...
              <div class="tree-slots">
                ...
                >
                  <div
                    ...
                    :title="
                      perksMap.get(perkId) ? perksMap.get(perkId).name : ''
                    "
                  >
          ...
          <div class="rune-tree-right">
            <div class="rune-presets-box">
              <div
                v-for="(r, i) in buildData.runes.slice(0, 3)"
                :key="i"
      ...
      <!-- 技能加点：主技能图标 + 升级顺序 + 胜率 -->
      <div
        v-if="buildData.skills?.length || buildData.skill_masteries?.length"
        class="build-card skill-card"
      >
        <div class="skill-section">
          <div class="skill-main">
            <template
              v-for="(sid, j) in buildData.skill_masteries?.[0]?.ids || []"
              :key="j"
            >
              ...
              <span
                v-if="
                  j < (buildData.skill_masteries?.[0]?.ids?.length || 0) - 1
                "
                class="skill-arrow"
                >›</span
              >
          ...
          <div class="skill-order" v-if="buildData.skills?.[0]?.order">
            <span
              v-for="(s, j) in buildData.skills[0].order"
              :key="j"
          ...
          <div class="skill-stats">
            <span class="skill-wr">{{
              pct(buildData.skills?.[0]?.win / buildData.skills?.[0]?.play)
            }}</span>
            <span class="skill-games">{{
              $t("career.gamesCount", { count: buildData.skills?.[0]?.play })
            }}</span>
          </div>
          <span class="skill-pick">{{
            pct(buildData.skills?.[0]?.pick_rate)
          }}</span>
        </div>
      </div>
      ...
      <div
        v-if="
          buildData.boots?.length ||
          buildData.core_items?.length ||
          buildData.starter_items?.length ||
          buildData.last_items?.length
        "
      ...
      >
        ...
        <div
          class="item-row-flex"
          v-if="buildData.starter_items?.length || buildData.boots?.length"
        >
          <div class="item-group">
            <div
              v-for="(item, i) in (buildData.starter_items || []).slice(0, 3)"
              :key="'s' + i"
              class="item-entry"
            >
              <LcuImage
          ...
          <div class="item-group">
            <div
              v-for="(item, i) in (buildData.boots || []).slice(0, 3)"
              :key="'b' + i"
              class="item-entry"
            >
              <LcuImage
        ...
        <!-- 核心装备（图标链 + 箭头 + 胜率） -->
        <div
          v-for="(item, i) in (buildData.core_items || []).slice(0, 5)"
          :key="'c' + i"
        ...
        <div v-if="buildData.last_items?.length" class="last-items-row">
          <LcuImage
            v-for="(item, i) in buildData.last_items.slice(0, 10)"
            :key="'l' + i"
